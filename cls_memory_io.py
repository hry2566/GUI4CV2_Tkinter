...
class MemoryIO(EditWindow):
    def __init__(self, img, param, master=None, gui=False):
        ...
        if type(self.origin_img) == list:
            img = self.origin_img[0]
    ...
    def __init_gui(self):
        ...
        if type(self.img_bk) == np.ndarray:
    ...
    def __memory_io(self):
        ...
        menu_val = self.__memIO[0]
        if menu_val == 'source_IMG':
            cnt = 0

            for cnt in range(len(self.__memIO[1])):
                name = self.__memIO[1][cnt]
                if name == '':
                    break
                if name.startswith('source_IMG['):
                    index = int(name.replace(
                        'source_IMG[', '').replace(']', ''))
                    self.img_dummy.append(self.img_bk[index])
                else:
                    index = self.__img_names.index(name)
                    self.img_dummy.append(self.__img_array[index])
                cnt += 1
        else:
            in_name = self.__memIO[0]
            if in_name.startswith('source_IMG['):
                name_index = int(in_name.replace(
                    'source_IMG[', '').replace(']', ''))
                self.__img_array[self.__img_names.index(
                    menu_val)] = self.img_bk[name_index]
            else:
                self.__img_array = []
                self.__img_array.append(img)

        if type(self.img_dummy) == list:
            if len(self.img_dummy) == 1:
                img = self.img_dummy[0]
            else:
                img = self.img_dummy
...
if __name__ == "__main__":
    ...
    imgLib = MemoryIO(img, param, gui=False)
    param, img = imgLib.get_data()
    img_array = param[0]
    img_names = param[1]

    cv2.imwrite('./MemoryIO1.jpg', img_array[0])
